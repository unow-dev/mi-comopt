...
- 実DB `var/comment-history.sqlite3` の移行前バックアップを `var/comment-history.sqlite3.bak-20260916-042815` として取得し、取得時点の元DBとSHA-256 `dfbd6b3d2e8312ab3aebc483ea70d9f58c615dd75fb3c6936815c642c231cea6` が一致することを確認した。
- `package/scripts/migrate-state-control-plane.mjs` と `migrate:state-control-plane` npm scriptを追加し、current keyword publicationが参照するraw snapshotを移行元として、互換Policy 7件、Corpus Genesis、Classification Genesis、Keyword Selection Genesisを冪等に作成できるようにした。
- 実DBへ移行を適用し、snapshot 9（payload SHA-256 `98b1a2d6ca08fe61b8b821d4498e8bab8683d4cf0c4ac0843cb5b89f7a014c7c`、観測24,622件）、分類label 24,622件、current keyword publication `run_8c7e7beb-3acd-497d-8615-ea1a8f71dc18` の候補196件をState Control Planeへ取り込んだ。Classificationのsemantic fingerprintは `d77ae948a27c22318f00419543b37650363accd1463b506680d96edc694170c7`、Keyword Selectionは `d3882d8e1a55c08561a0ac308b12e8cd2eacb6b7953896430569cc2d1e5eb779` で、いずれも同値性検証に成功した。
- 実DBはState Control Plane 12 stream・10 version・7 policyを持ち、Corpus/Classification/Keyword Selectionの3 streamを `Verified`（legacy writer有効）まで進めた。`PRAGMA integrity_check=ok`、user_version 8、current publication維持を確認し、同一移行を再実行しても状態が変わらないことを確認した。
- Cutover、Legacy Read Compatibility、Retiredは実施していない。12.1・12.2およびClassification/Keyword Selectionの人間による切替判断（7.3・8.3）、全streamのRetired判断（12.3）は、人間がgateと運用影響を確認した後に継続する。
- 本番の人間承認手順は明示的な依頼によりdemo検証では省略し、実DBのCutoverは実施しない方針へ変更した。代わりに、実DBをコピーした分離demo DB上で、PromotionのHuman Taskを`demo-reviewer`が自動承認する業務フロー検証を追加した。
