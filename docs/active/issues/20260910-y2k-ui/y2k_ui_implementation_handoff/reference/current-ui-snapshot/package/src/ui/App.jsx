...
function LabelSummaryCard({ summary }) {
  return (
    <section className="label-summary" aria-labelledby="label-summary-heading">
      <div className="label-summary__header">
        <div>
          <span className="label-summary__eyebrow">集計対象</span>
          <h2 id="label-summary-heading">{summary.total.toLocaleString("ja-JP")}件のコメント</h2>
        </div>
        ...
        {LABEL_SUMMARY_ROWS.map((row) => {
          ...
          return (
            <div className={`label-summary__row label-summary__row--${row.tone}`} key={row.key}>
              ...
              <div className="label-summary__bar" aria-hidden="true">
  ...
  return (
    <main className="page-shell">
      <section className="view-switcher" aria-label="表示切替">
        ...
        <button
          className={view === "labels" ? "view-switcher__button view-switcher__button--active" : "view-switcher__button"}
          type="button"
      ...
      <section className="hero">
        <div>
          {view === "keywords" ? (
            ...
            <>
              <p className="eyebrow">THREE-CLASS LABEL SUMMARY</p>
              <h1>コメントラベル集計</h1>
              <p className="description">Comment DBに登録された3分類ラベルの内訳です。集計元のスナップショットを明示しています。</p>
            </>
          )}
        </div>
      </section>
