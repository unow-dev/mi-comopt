...
function LogoMark({ compact = false }) {
  return <span className={`grid place-items-center rounded-xl bg-[linear-gradient(145deg,#4EC8F7,#2BB1EA_55%,#FF5F98)] font-black text-white shadow-[0_8px_18px_rgba(46,173,229,.22)] ${compact ? 'h-8 w-8 text-xs' : 'h-11 w-11 text-base'}`}>O+</span>
}

function Brand({ compact = false }) {
  return <div className={`flex items-center ${compact ? 'gap-2' : 'gap-3'}`} aria-label="こちらコメント管理局！">
    <LogoMark compact={compact} />
    <div className="whitespace-nowrap text-[13px] font-black tracking-[-.08em] text-ink">こちらコメント<span className="text-pink">管理局！</span>{!compact && <small className="mt-1 block text-[9px] font-bold tracking-[.08em] text-[#7A91AE]">いいね！わるいね！削除だね！</small>}</div>
  </div>
}
...
function Header({ screen, onChange, dataEndDate }) {
  const title = screen === 'home' ? <><span className="max-[820px]:hidden">ホーム</span><span className="hidden items-center gap-1.5 max-[820px]:inline-flex"><LogoMark compact /><span>こちらコメント<span className="text-pink">管理局！</span></span></span></> : screenTitles[screen]
  return <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between gap-4 border-b border-[rgba(220,234,244,.92)] bg-[rgba(250,253,255,.92)] px-7 backdrop-blur-[18px] max-[820px]:h-16 max-[820px]:px-3.5">
    <div className="flex min-w-0 items-center gap-2.5">
      ...
      <div className="min-w-0"><strong className="block truncate text-lg font-black leading-[1.1] tracking-[-.025em] max-[820px]:text-base">{title}</strong><span className="mt-1 block text-[10px] text-[#8A9AB0] max-[820px]:hidden">Comment DB公開データ</span></div>
    </div>
...
function CommentsScreen({ artifact }) {
  ...
  return <section className="animate-screen"><SectionHeader title="コメント一覧" description="投稿日順（日単位）で対象観測を確認します。" right={<Pill>{artifact.records.length.toLocaleString('ja-JP')}件</Pill>} /><div className="mb-4 flex flex-wrap gap-2">{[['all', 'すべて'], ...LABELS.map(({ key, label: name }) => [key, name])].map(([key, name]) => <button key={key} type="button" onClick={() => setLabel(key)} className={`rounded-full border px-[13px] py-[9px] text-[10px] font-black ${label === key ? 'border-[#8ED9F8] bg-[#EAF9FF] text-[#2298D3]' : 'border-line bg-white text-[#637896]'}`}>{name}{key !== 'all' && ` ${artifact.records.filter((record) => record.label === key).length.toLocaleString('ja-JP')}`}</button>)}</div><div className="mb-3.5 flex items-center gap-[9px]"><input value={query} onChange={(event) => setQuery(event.target.value)} className="h-[42px] min-w-[260px] max-w-[520px] flex-1 rounded-[13px] border border-line bg-white px-[13px] text-ink outline-0 focus:border-[#72CCF4] max-[560px]:min-w-0 max-[560px]:text-base" placeholder="コメント・ユーザー名・ハンドルを検索" aria-label="コメントを検索" /><span className="text-[10px] font-black text-[#8392A7]">{filtered.length.toLocaleString('ja-JP')}件</span></div><div className="grid gap-2.5">{paged.records.map((record) => <article key={`${identity}-${record.source_index}`} className={`${card} grid grid-cols-[auto_1fr] items-start gap-[13px] p-4 max-[560px]:p-3.5`}><div className="grid h-[42px] w-[42px] place-items-center rounded-[14px] bg-[#ECF8FD] text-[#2A91C2]"><Icon name="user" /></div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="text-xs">{record.username}</strong><span className="text-[10px] text-[#8A9AB0]">{record.handle}</span><Pill tone={LABELS.find((item) => item.key === record.label)?.tone}>{labelName(record.label)}</Pill></div><p className="mt-2 break-words text-xs leading-[1.7] text-[#536985]">{record.comment}</p><div className="mt-2 text-[10px] text-[#8A9AB0]">投稿日: {record.postedDate} / 取得値: {record.postedAt}</div></div></article>)}{paged.records.length === 0 && <div className={`${card} border-dashed p-10 text-center text-xs text-[#8A9AB0]`}>条件に一致する観測はありません。</div>}</div><div className="mt-4 flex items-center justify-center gap-3"><button type="button" disabled={paged.page <= 1} onClick={() => setPage((value) => value - 1)} className={`${button} border border-line bg-white text-[#607592]`}>前へ</button><span className="text-[10px] font-black text-[#7B8EA7]">{paged.page} / {paged.totalPages}</span><button type="button" disabled={paged.page >= paged.totalPages} onClick={() => setPage((value) => value + 1)} className={`${button} border border-line bg-white text-[#607592]`}>次へ</button></div></section>
}
