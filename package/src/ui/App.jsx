...
function KeywordCard({ item, hidden, onCopy, now }) {
  ...
  const variants = (item.variants ?? []).filter(variant => variant !== item.keyword);
  ...
  return (
    <article className='candidate-card' hidden={hidden}>
      ...
      <div className='badges' aria-label='分類'>
        <span className={'badge ' + recommendationClass}>{item.recommendation}</span>
      ...
      <div
        ...
        hidden={!open}
      >
        <DetailRow label='direct（命中）' value={item.directNuisanceHits + '件'} />
        <DetailRow label='reactive（参考）' value={item.reactiveHits + '件'} />
        <DetailRow label='normal（誤爆）' value={item.normalHits + '件'} />
        <DetailRow
          label='精度（reactive除外）'
        ...
        {item.matchType && <DetailRow label='マッチ方式' value={item.matchType} />}
      </div>
    </article>
  );
}
  ...
  return (
    <article className='account-card'>
      <div className='account-head'>
        <div>
          ...
          <p className='account-meta'>
            direct_nuisance <span className='count'>{item.directNuisanceCount}件</span>
          </p>
        </div>
        <CopyButton
          value={item.handle}
      ...
      >
        ...
        <DetailRow label='該当コメント数' value={item.directNuisanceCount + '件'} />
        ...
        <ol className='account-evidence-list'>
          {evidence.map((example, index) => (
            <li key={example.postedDate + '-' + example.postedAt + '-' + index}>
...
function KeywordResults({ visibleCandidateIds, onCopy, now }) {
  return (
    <>
      ...
      <div className='empty' id='emptyState' hidden={visibleCandidateIds.size > 0}>
        条件に一致する候補はありません。
      </div>
    </>
  );
            ...
            <article className={'label-card ' + row.className} key={row.key}>
              <div className='label-head'>
                <div className='label-copy'>
                  <span className={'badge ' + row.badgeClassName}>{row.label}</span>
                  <p className='label-description'>{row.description}</p>
                  <p className='label-stat'>
                    <span className='label-count'>{count.toLocaleString('ja-JP')}件</span>
    ...
    return new Set(
      keywordCandidates
        .filter(item => {
          ...
          const matchesNew =
            !newOnly ||
            isNewCandidate(item.introducedAt, now, workflowConfig.newKeywordDisplayDays);
  ...
  return (
    <>
      <main className='app-shell'>
        <TabsPrimitive.Root className='app-tabs' value={view} onValueChange={setView}>
            ...
            <section className='page-card' aria-labelledby='page-title'>
              ...
              <div className='content'>
                ...
                <div className='data-note'>
                  <b>集計データ</b>
                  <br />
                  <code>src/data/threeClassLabelSummary.json</code>{' '}
                  を参照する画面として構成しています。表示値はスナップショット単位で固定し、3分類の合計と集計対象件数が一致する前提です。
                </div>
              </div>
            </section>
