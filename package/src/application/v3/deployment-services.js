...
import { STREAM_KEYS, readExactPolicyVersion } from "../services.js";
...
export class DeploymentQueueServiceV3 {
  ...
  async trigger(ctx, request = {}) {
    ...
    const earlier = this.controlPlane.db.prepare("SELECT * FROM v3_deployment_requests WHERE target = ? AND deployment_sequence < ? AND status NOT IN ('succeeded','failed','cancelled','superseded') ORDER BY deployment_sequence LIMIT 1").get(row.target, row.deployment_sequence);
    if (earlier) return v3StepResult({ stepId: ctx.stepId, routingOutcome: "wait", stateResult: "triggered", refs: { deploymentRequestId: row.deployment_request_id, releaseId: row.release_id, promotionVersionId: row.promotion_version_id }, details: { waitingFor: earlier.deployment_request_id } });
    const latestPromotion = promotionHead(this.controlPlane);
    if ((latestPromotion.head?.versionId ?? null) !== row.promotion_version_id) {
      this.controlPlane._transaction((db) => db.prepare("UPDATE v3_deployment_requests SET status = 'superseded', updated_at = ? WHERE deployment_request_id = ? AND status = 'queued'").run(this.controlPlane.now(), row.deployment_request_id));
      return v3StepResult({ stepId: ctx.stepId, routingOutcome: "superseded", stateResult: "conflict", refs: { releaseId: row.release_id, promotionVersionId: row.promotion_version_id } });
    }
    if (!this.adapter) throw stateError("CONFIGURATION_ERROR", "v3 deployment adapter is required");
    let ensured;
    try { ensured = await this.adapter.ensureDeployment({ deploymentRequestId: row.deployment_request_id, releaseId: row.release_id, target: row.target }); }
    catch (error) { throw stateError(error.code ?? "DEPLOYMENT_EXECUTION_FAILED", error.message, { cause: error }); }
    const status = statusFromAdapter(ensured);
    this.controlPlane._transaction((db) => db.prepare("UPDATE v3_deployment_requests SET status = ?, external_run_ref = COALESCE(external_run_ref, ?), updated_at = ? WHERE deployment_request_id = ? AND status NOT IN ('succeeded','failed','cancelled','superseded')").run(status, ensured?.externalRunRef ?? null, this.controlPlane.now(), row.deployment_request_id));
    const final = deploymentRow(this.controlPlane, row.deployment_request_id);
  ...
  async deliverPendingEvents(deliver, { registry = undefined, limit = 50, maxAttempts = 5 } = {}) {
    ...
    for (const row of rows) {
      ...
      const receipt = typeof registry?.getReceipt === "function" ? registry.getReceipt(row.command_id) : null;
      try {
        if (receipt) {
  ...
  record(ctx, request = {}) {
    ...
    const policy = readExactPolicyVersion(this.controlPlane, request.deploymentPolicyVersionId, "deployment-production");
    ...
    this.controlPlane.createProposal({ proposalId, streamId: stream.stream_id, expectedHeadVersionId: head?.versionId ?? null, proposedSemanticSha256: semanticSha256(state), payload: { schema_version: 1, state }, dependencies: [{ role: "policy", versionId: policy.versionId }], assessmentRefs: { verificationRef: request.verificationRef }, operationId: `${ctx.operationId}/proposal` });
    const decision = this.controlPlane.createDecision({ decisionId: deterministicId("decision-v3", `${ctx.operationId}:deployment`), proposalId, outcome: "accepted", authorityKind: "system_policy", authorityRef: "deployment-policy", transitionPolicyVersionId: policy.versionId, operationId: `${ctx.operationId}/decision` });
    try {
