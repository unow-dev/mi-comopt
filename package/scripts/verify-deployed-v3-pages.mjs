...
export async function verifyDeployedV3Pages({ url, deploymentRequestId, releaseId, releaseBundleSha256, fetchImpl = globalThis.fetch } = {}) {
  ...
  const markerUrl = new URL("comment-db-v3-deployment.json", url.endsWith("/") ? url : `${url}/`);
  ...
  return { status: "verified", url: markerUrl.href, releaseId, deploymentRequestId, markerSha256: prefixedSha256(bytes) };
}
